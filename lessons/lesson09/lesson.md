...
- Понять различие между React-элементами и компонентами
- Научиться рендерить элементы через `react-dom`
- Разобраться с базовыми правилами JSX и новой JSX-трансформацией
- Освоить базовые подходы к тестированию компонентов (RTL, выбор элементов)
- Кратко рассмотреть инструменты и экосистему (CRA — исторический контекст, Storybook, Loki)

<!-- v -->

## Краткое содержание

- Что такое React и зачем он нужен (идея скорости, Virtual DOM)
- Элементы vs компоненты; `React.createElement` и JSX
- Рендеринг через `ReactDOM.createRoot().render(...)`
- Правила и практики JSX
- Тестирование компонентов: RTL, выбор элементов, fireEvent vs userEvent
- Инструменты и стартовые шаблоны (CRA — не используем на курсе), Storybook, Loki

<!-- v -->

## Результат занятия

Мини‑приложение на React с компонентом на JSX + базовые примеры тестов на React Testing Library
...
- Разрабатывать и переиспользовать React‑компоненты
- Использовать современный инструментарий для фронтенда
- Понимать основы тестирования React‑компонентов
