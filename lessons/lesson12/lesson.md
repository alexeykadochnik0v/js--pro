...
- **`@types/react`, `@types/react-dom`**: пакеты с _декларациями типов_. Без них TypeScript не будет знать, что такое `useState`.
...
const Title: React.MemoExoticComponent<(props: Props) => JSX.Element> =
  React.memo(function Title({ title }) {
    return <h1>{title}</h1>;
  });
```
  ...
  return (
    <div>
      ...
      <button onClick={handleClick}>
        Programmatically click the above button
      </button>
    </div>
  );
};
...
const handleClick = () => {
  if (inputRef.current) {
    // TypeScript знает, что здесь current - HTMLInputElement
...
const badHandler = useCallback((event) => {
